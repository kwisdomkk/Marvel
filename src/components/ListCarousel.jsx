...
const ListItem=({item,CARD_WIDTH,CARD_HEIGHT,MARGIN})=>(
  <div className="shrink-0 group cursor-pointer" style={{width:CARD_WIDTH,height:CARD_HEIGHT,margin:MARGIN}}>
      {/* 이미지 박스 */}
      <div className="w-full h-[280px]">
        <img className="w-full h-full object-cover object-center group-hover:-translate-y-3 duration-300"
          src={`${item.thumbnail?.path}.${item.thumbnail?.extension}`} alt="comic_image"/>
      </div>
      {/* 타이틀 */}
      <div className="">
        <h2 className="text-sm font-semibold duration-300 group-hover:text-red-600">{item.title?item?.title.substr(0,20):item?.name.substr(0,20)}</h2>
